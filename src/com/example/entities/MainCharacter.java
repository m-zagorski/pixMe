...
public class MainCharacter implements Characters {
	...
	public MainCharacter(OurView ourView, Bitmap bitmap, int x, int y,
			int level, String charClass, int[] characterSkills) {
	...
	public void onDraw(Canvas canvas) {
		if (animateSkill) {
			animate();
		} else if ((doAttack && characterClass.equals("wizard"))
				|| (doAttack && characterClass.equals("hunter"))) {
			rangedAttack();
		} else if (doAttack && characterClass.equals("barbarian")) {
			update();
		} else {
			if (health > 0) {
				direction = 0;
	...
	private void update() {
		if (state == State.ONE) {
			...
			if (characterClass.equals("barbarian") && thirdSkill) {
				direction = 6;
			} else if (characterClass.equals("wizard") && thirdSkill) {
				direction = 9;
			} else if (characterClass.equals("hunter") && thirdSkill) {
				direction = 8;
			} else {
				direction = 6;
			}
	...
	private void rangedAttack() {
		...
		if (characterClass.equals("wizard") && thirdSkill) {
			direction = 8;
		} else if (characterClass.equals("hunter") && thirdSkill) {
			direction = 7;
		} else {
