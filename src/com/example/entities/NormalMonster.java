...
public class NormalMonster implements Monsters {
	...
	public NormalMonster(OurView ourView, Bitmap bitmap, int x, int y,
			String monsterType, long health, int armor, int damage,
			int playerLevel, String coords) {
		this.damage = damage;
		this.health = health;
	...
	public void onDraw(Canvas canvas) {
		...
		if (monsterTurn) {
			...
			if (health > 0) {
				direction = Integer.parseInt(this.coords[1]);
			} else {
				direction = Integer.parseInt(this.coords[5]);
			}
			framing();
		}
